...
const isOwnerOrAdmin = (resourceUserId: string, authUser: IAuthUser): boolean =>
  resourceUserId === authUser.userId || authUser.role === 'ADMIN';

const computeNextStatus = (remainingAmount: number, dueDate: Date | null): LoanStatus => {
  if (remainingAmount <= 0) return LoanStatus.FINISHED;
  if (dueDate && dueDate < new Date()) return LoanStatus.DUE;
  return LoanStatus.ACTIVE;
};

const ensureLoanAccess = async (loanId: string, authUser: IAuthUser) => {
...
const createTransaction = async (
  ...
  return prisma.$transaction(async (tx) => {
    if (payload.type === 'PAYMENT' && payload.amountPaid > loan.remainingAmount) {
      throw new ApiError(400, `Payment amount cannot exceed the remaining balance of ${loan.remainingAmount}`);
    }
...
const getTransactionById = async (id: string, authUser: IAuthUser): Promise<unknown> => {
  ...
  if (!isOwnerOrAdmin(transaction.userId, authUser)) {
    throw new ApiError(403, 'Forbidden: you do not own this transaction');
  }
...
const updateTransaction = async (
  ...
  const loan = await prisma.loan.findUnique({ where: { id: transaction.loanId } });
  if (!loan) {
    throw new ApiError(404, 'Loan not found');
  }
  ...
  if (newRemaining < 0) {
    throw new ApiError(400, 'Transaction update would make the loan balance negative');
  }
...
const deleteTransaction = async (id: string, authUser: IAuthUser): Promise<void> => {
  const transaction = await prisma.transaction.findUnique({ where: { id } });
  if (!transaction) {
    throw new ApiError(404, 'Transaction not found');
  }
...
const getStats = async (authUser: IAuthUser): Promise<unknown> => {
  ...
  const [aggregate, payments, deposits, count, recent] = await Promise.all([
    prisma.transaction.aggregate({ where: scope, _sum: { amountPaid: true }, _count: { _all: true } }),
    prisma.transaction.aggregate({
      where: { ...scope, type: TransactionType.PAYMENT },
      _sum: { amountPaid: true },
    }),
    prisma.transaction.aggregate({
      where: { ...scope, type: TransactionType.DEPOSIT },
      _sum: { amountPaid: true },
    }),
